<script lang="ts">
	...
	async function runMigration() {
		...
		try {
			await runFullMigration(isDryRun);
			completed = true;
		} catch (error) {
			hasError = true;
...
<div class="min-h-screen bg-gray-50 p-8">
	<div class="max-w-6xl mx-auto">
		...
		<div class="bg-white rounded-lg shadow-sm p-6 mb-6">
			<h1 class="text-3xl font-bold text-gray-900 mb-2">V1 → V2 Migration</h1>
			<p class="text-gray-600">
				Migrate existing contracts and clients to the new v2 architecture.
			</p>
					...
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<div class="text-sm text-red-700 mt-1">
								This will write data to your Firestore database. Make sure you have a backup before proceeding.
							</div>
						</div>
					</div>
				</div>
				...
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<div class="text-sm text-green-700 mt-1">
							...
							{:else}
								All data has been migrated to v2 collections. Review the logs for details.
							{/if}
						</div>
					</div>
				</div>
		...
		<!-- Logs -->
		{#if logs.length > 0}
			<div class="bg-gray-900 rounded-lg shadow-sm p-6 overflow-auto">
