<script lang="ts">
	import { fixCounterpartyNullValues } from '$lib/migration/fixCounterpartyNullValues';
	...
	async function runMigration() {
		...
		try {
			const migrationResult = await fixCounterpartyNullValues(isDryRun);
...
<div class="p-8">
	<div class="max-w-6xl mx-auto">
		...
		<div class="bg-white rounded-lg shadow-sm p-6 mb-6">
			<h1 class="text-3xl font-bold text-gray-900 mb-2">Fix Null Values in Counterparties</h1>
			<div class="text-gray-600">
				<p class="mb-2">Fix null values in counterparty documents:</p>
				<ul class="list-disc list-inside space-y-1">
					<li>
						Convert <code class="px-1.5 py-0.5 bg-gray-100 rounded text-sm">null</code> arrays to
						empty arrays <code class="px-1.5 py-0.5 bg-gray-100 rounded text-sm">[]</code>
					</li>
					<li>
						Set null/empty <code class="px-1.5 py-0.5 bg-gray-100 rounded text-sm">serviceType</code
						> to "N/A"
					</li>
				</ul>
			</div>
			...
			<div class="flex items-center gap-6">
				...
				<label class="flex items-center gap-3 cursor-pointer">
					...
					<div>
						...
						<div class="text-sm text-gray-500">Preview changes without writing to database</div>
					</div>
				</label>
					...
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<div class="text-sm text-red-700 mt-1">
								This will write data to your Firestore database. Make sure you have a backup before
								proceeding.
							</div>
						</div>
					</div>
				...
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<div class="text-sm text-green-700 mt-1">
							...
							{:else}
								All null values have been fixed. Review the logs for details.
							{/if}
						</div>
					</div>
				</div>
			...
			<div class="bg-red-50 border border-red-200 rounded-lg p-6 mb-6">
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<div class="text-sm text-red-700 mt-1">Check the error logs below for details.</div>
					</div>
				</div>
			</div>
		{/if}
