...
export class EquipmentRentalContractState {
	...
	init() {
		...
		try {
			this.unsubscribe = subscribeToEquipmentRentalContracts(
				(contracts) => {
	 ...
	 */
	get unpaid(): EquipmentRentalContract[] {
	 ...
	 */
	getByEvent(eventId: string): EquipmentRentalContract[] {
	 ...
	 */
	getByCounterparty(counterpartyId: string): EquipmentRentalContract[] {
		return this.contracts.filter((c) => c.counterpartyId === counterpartyId);
	}
}

export const equipmentRentalContractState = new EquipmentRentalContractState();
