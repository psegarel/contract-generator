<script lang="ts">
	...
	let allContracts = $derived<BaseContract[]>(
		[
			...
			...equipmentRentalOneOffContractState.contracts
		]
			.sort((a, b) => {
	...
	let isLoading = $derived(
		serviceProvisionContractState.isLoading ||
			...
			equipmentRentalOneOffContractState.isLoading
	);
	...
	let hasAnyContracts = $derived(
		serviceProvisionContractState.contracts.length > 0 ||
			...
			equipmentRentalOneOffContractState.contracts.length > 0
	);
	...
	let hasErrors = $derived({
		...
		equipmentRentalOneOff: equipmentRentalOneOffContractState.error !== null
	});
		...
		{#if hasErrors.serviceProvision || hasErrors.eventPlanning || hasErrors.equipmentRental || hasErrors.equipmentRentalOneOff}
			<div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-800">
				...
				<ul class="list-disc list-inside space-y-1">
					...
					{/if}
					{#if hasErrors.equipmentRentalOneOff}
						<li>Equipment Rental (One-Off): {equipmentRentalOneOffContractState.error}</li>
					{/if}
				</ul>
			</div>
		{/if}
