<script lang="ts">
	...
	// Derived initialData for ClientForm to ensure reactivity
	// Track all formData properties explicitly to ensure the derived value updates when formData changes
	let clientInitialData = $derived.by(() => {
		if (!editContractId) return undefined;
		// Access all properties to ensure they're tracked, including bank fields
		// Use nullish coalescing to preserve empty strings and convert null/undefined to empty string
		return {
			...
			// Ensure bank fields are always strings (not null) for proper input display
	...
	onMount(async () => {
		...
		if (editId) {
			...
			try {
				...
				if (contract) {
					// Ensure all fields are properly copied, including bank fields
					formData = { 
						...contract.contractData,
						// Explicitly ensure bank fields are strings (not null/undefined)
						bankName: contract.contractData.bankName ?? '',
						accountNumber: contract.contractData.accountNumber ?? ''
					};
					...
					goto('/contracts/service-provision');
				}
			} catch (error) {
