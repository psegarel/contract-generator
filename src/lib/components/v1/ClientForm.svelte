<!--
	...
	Use $lib/components/v2/counterparties/ClientForm.svelte instead for new development.
	This component is only kept for backward compatibility with v1 service contracts.
-->
<script lang="ts">
	...
	function createInitialFormData(data: ClientData | undefined): ClientData {
		...
		return {
			...
			// Convert null/undefined to empty string for bank fields to ensure they display in inputs
	...
	// Create a serialized key from initialData to track changes
	// This allows us to re-initialize formData when initialData changes without using $effect
	let initialDataKey = $derived(
		props.initialData
			? JSON.stringify({
					...
					idDocument: props.initialData.idDocument,
	...
	// The key prop in parent component will force re-initialization when initialData changes
	let formData = $state<ClientData>(createInitialFormData(props.initialData));

	// Re-initialize formData when initialData changes
	// This is necessary for async data loading scenarios
	...
	$effect(() => {
		...
		const _key = initialDataKey;
		// Re-initialize formData when initialData changes
		if (props.initialData) {
			const initial = props.initialData;
			// Update all fields, ensuring bank fields are strings (not null) for proper input display
			formData.name = initial.name || '';
			formData.email = initial.email || '';
			formData.phone = initial.phone || '';
			formData.address = initial.address || '';
			formData.idDocument = initial.idDocument || '';
			formData.taxId = initial.taxId || null;
			// Use nullish coalescing to convert null/undefined to empty string, preserve empty strings
